...
const FormAuth = <T extends object>({ title, subTitle, inputs, btn, footer, setData }: FormAuthProps<T>) => {
	...
	const handelData = (event: ChangeEvent<HTMLInputElement>, name: string, type: string) => {
		const value = type !== "file" ? event.target.value : event.target.files?.[0];
		...
		<form className="flex flex-col min-h-[547px] items-center" onSubmit={sendData}>
			...
			<div className={`w-full grow flex flex-col gap-8 ${inputs.length > 3 ? "grid grid-cols-6 gap-x-4 gap-y-2" : ""}`}>
				{inputs.map((input, index) => (
					<div key={index} className={`min-w-[304px] flex flex-col items-start col-span-2 ${input.type === "email" ? "col-span-6 col-start-1 col-end-7" : ""
						} ${input.type === "password" ? "col-span-3" : ""
						}`}>
						...
						<input
							...
							id={"input" + index}
							className={`w-full h-11 rounded-lg bg-gray-200 border border-gray-300 px-4 text-[15px] outline-0 ${input.type === "file" ? "hidden" : ""}`}
